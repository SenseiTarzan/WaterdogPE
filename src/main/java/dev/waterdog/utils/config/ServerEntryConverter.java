...
public class ServerEntryConverter implements Converter {
    ...
    public Object fromConfig(Class<?> type, Object object, ParameterizedType parameterizedType) throws Exception {
        ...
        if (object instanceof ConfigSection) {
            ...
            serverType = (String) inetConverter.fromConfig(String.class, section.get("public_address"), null);
            return new ServerEntry(section.get("name"), address, publicAddress, serverType);
        }
        ...
        if (object instanceof Map) {
            ...
            for (Map.Entry<String, Map<String, ?>> subMap : map.entrySet()) {
                ...
                serverType = (String) inetConverter.fromConfig(String.class, subMap.getValue().get("public_address"), null);
                return new ServerEntry(subMap.getKey(), address, publicAddress, serverType);
            }
        }
